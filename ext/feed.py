...
def create_items(app, pagename, templatename, ctx, doctree):
   ...
   etree.SubElement(item, 'author').text = postAuthor + '@mpc-hc.org ('+ postAuthor +')'
   etree.SubElement(item, 'link').text = "%s/%s" % (app.config.base_uri, app.builder.get_target_uri(pagename))
   guid = etree.SubElement(item, 'guid')
   guid.text = hashlib.sha1(ctx.get('body').encode('utf-8')).hexdigest()
   guid.set('isPermaLink', 'false')
   etree.SubElement(item, 'description').text = '\n'.join(ctx.get('body').split('\n')[3:-2])
...
def write_feed(app, exc):
   ...
   container[:] = sorted(container, key=lambda i: i.findtext('date'), reverse=True)
